...
//		add(successLabel);   
  ...
  public void Display()
  {
	  url = "jdbc:mysql://localhost:3306/A_R_TRAVELS";
		...
		qry="SELECT Fullname,Gender,Address,Contact,Adharno,Payment_Amount,Payment_Method from tbl_details order by details_ID DESC limit 1;";
...
//		System.out.println("QRY"+Rs.next());
		...
		Rs.next();
		a = Rs.getInt(1);
		qry ="Select TourType_Name From tbl_tourtype where TourType_ID="+a;
		Stm = Con.createStatement();
		Rs=Stm.executeQuery(qry);
		while(Rs.next())
		{
			DPackageType = new JLabel("PaCkage Type"+Rs.getString(1));
		}
		qry = "Select Package_ID from tbl_details order by details_ID DESC limit 1;";
		Stm = Con.createStatement();
		Rs=Stm.executeQuery(qry);
		int b;
		b = Rs.getInt(1);
		qry ="Select Destination From tbl_packages where Package_ID="+b;
		Stm = Con.createStatement();
		Rs=Stm.executeQuery(qry);
		while(Rs.next())
		{
			DPackageName = new JLabel("PaCkage name"+Rs.getString(1));
		}
		qry = " Select Month_ID from tbl_details order by details_ID DESC limit 1;";
		Stm = Con.createStatement();
		Rs=Stm.executeQuery(qry);
		int c;
		Rs.next();
		c = Rs.getInt(1);
		qry ="Select Month_Name From tbl_month where Month_ID="+c;
		Stm = Con.createStatement();
		Rs=Stm.executeQuery(qry);
		while(Rs.next())
		{
			DMonth = new JLabel("Month"+Rs.getString(1));
		}
		qry = "Select Operator_ID from tbl_details order by details_ID DESC limit 1;";
		Stm = Con.createStatement();
		Rs=Stm.executeQuery(qry);
		int d;
		Rs.next();
		d = Rs.getInt(1);
		qry ="Select Operator_Name From tbl_operator where  Operator_ID="+d;
		Stm = Con.createStatement();
		Rs=Stm.executeQuery(qry);
		while(Rs.next())
		{
			Doprator = new JLabel("Opreator"+Rs.getString(1));
		}
		//,, ,,
